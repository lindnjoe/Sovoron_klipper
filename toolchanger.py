...
class Toolchanger:
    ...
    def _handle_ready(self):
        # Check initial tool detection states after all components are initialized
        # This is needed because button callbacks only fire on state CHANGES,
        # so if a tool is already mounted at boot, we need to check the current state
        if self.has_detection:
            logging.info("Toolchanger: Checking initial tool detection states")
            # Force re-evaluation of detected tool based on current pin states
            self.note_detect_change(None)
            if self.detected_tool:
                logging.info("Toolchanger: Detected tool at startup: %s" % self.detected_tool.name)
            else:
                logging.info("Toolchanger: No tool detected at startup")
