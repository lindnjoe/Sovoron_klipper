...
def test_afcams_lane_temperature_uses_saved_file(tmp_path):
    ...
    class DummyCache:
        ...
        def cache_lane_temp(self, lane, temp):
            if lane is None:
                return
            if temp is None:
                self.cached.pop(lane, None)
            else:
                self.cached[lane] = temp

        def get_lane_temp(self, lane, default):
            return self.cached.get(lane, default)
    ...
    assert result == 220
    assert helper.temp_cache.cached.get("lane8") == 220
