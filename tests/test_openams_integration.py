...
def test_afcams_prepare_unload_and_record_load(tmp_path):
    module = _afc_openams_module

    class DummyExtruder:
        def __init__(self, lane_loaded):
    ...
    class DummyPrinter:
        ...
        def lookup_object(self, name, default=None):
            if name == "AFC_extruder extruder4":
    ...
    extruder = DummyExtruder("lane7")
    printer = DummyPrinter(extruder)

    helper = object.__new__(module.afcAMS)
    helper.logger = logging.getLogger("test_afcams_prepare_unload")
    helper.printer = printer
    helper.afc = SimpleNamespace(VarFile=str(tmp_path / "AFC.var"), next_lane_load="lane8")
    helper.name = "AMS_1"
    helper.lanes = {
        "lane7": DummyLane("lane7", 220),
        "lane8": DummyLane("lane8", 230),
    }
    helper._lane_temp_cache = {}
    helper._last_loaded_lane_by_extruder = {}

    lane_name, temp = module.afcAMS.prepare_unload(helper, extruder="extruder4", default_temp=240)

    assert lane_name == "lane7"
    assert temp == 230
    assert helper._last_loaded_lane_by_extruder["extruder4"] == "lane7"
    assert helper._lane_temp_cache["lane7"] == 220
    assert helper._lane_temp_cache["lane8"] == 230

    purge_temp = module.afcAMS.get_purge_temp_for_change(
        helper,
        "lane7",
        "lane8",
        extruder="extruder4",
    ...
    assert purge_temp == 230

    recorded = module.afcAMS.record_load(helper, extruder="extruder4", lane_name="lane8")

    assert recorded == "lane8"
    assert helper._last_loaded_lane_by_extruder["extruder4"] == "lane8"
    assert helper._lane_temp_cache["lane8"] == 230
...
def test_pause_message_skips_pause_when_already_paused(monkeypatch):
    module = _oams_manager_module
    ...
    class DummyPauseResume:
        def __init__(self):
            self.is_paused = True

    class DummyPrinter:
        def __init__(self):
            self._reactor = SimpleNamespace(monotonic=lambda: 0.0)
            self._objects = {
                "gcode": DummyGCode(),
                "toolhead": DummyToolhead(),
                "pause_resume": DummyPauseResume(),
            }
    ...
    helper = object.__new__(module.OAMSManager)
    helper.printer = dummy_printer
    helper.reactor = dummy_printer.get_reactor()
    helper.logger = logging.getLogger("test_pause_message")

    original_coordinator = module.AMSRunoutCoordinator
    monkeypatch.setattr(module, "AMSRunoutCoordinator", None)
    try:
        helper._pause_printer_message("Test clog pause", oams_name=None)
    finally:
        setattr(module, "AMSRunoutCoordinator", original_coordinator)

    gcode_obj = dummy_printer.lookup_object("gcode")
    assert gcode_obj.scripts == [
        "M118 Print has been paused: Test clog pause",
        "M114 Print has been paused: Test clog pause",
    ]


def test_clog_pause_reenables_follower(monkeypatch):
    module = _oams_manager_module

    class DummyIdleTimeout:
        def get_status(self, _eventtime):
            return {"state": "Printing"}

    class DummyPrinter:
        def __init__(self):
            self._reactor = SimpleNamespace(monotonic=lambda: 100.0)
            self._objects = {"idle_timeout": DummyIdleTimeout()}
    ...
    class DummyFPS:
        def __init__(self, position):
            self.extruder = DummyExtruder(position)

    class DummyOAMS:
        def __init__(self):
            self.follower_calls = []
            self.led_calls = []
            self.hub_hes_value = [1, 1, 1]

        def set_oams_follower(self, enable, direction):
            self.follower_calls.append((enable, direction))

        def set_led_error(self, index, value):
            self.led_calls.append((index, value))

    dummy_oams = DummyOAMS()
    fps_name = "fps1"
    oams_name = "oams1"

    helper = object.__new__(module.OAMSManager)
    helper.printer = DummyPrinter()
    helper.reactor = helper.printer.get_reactor()
    helper.logger = logging.getLogger("test_clog_pause_follower")
    helper.oams = {oams_name: dummy_oams}
    helper.runout_monitors = {fps_name: SimpleNamespace(state=module.OAMSRunoutState.MONITORING)}
    helper.clog_settings = module.CLOG_SENSITIVITY_LEVELS[module.CLOG_SENSITIVITY_DEFAULT]
    helper.current_state = module.OAMSState()
    helper.current_state.fps_state[fps_name] = module.FPSState(
        state=module.FPSLoadState.LOADED,
        current_group="Group",
        current_oams=oams_name,
        current_spool_idx=2,
    )

    pause_messages = []
    helper._pause_printer_message = lambda message, oams_name=None: pause_messages.append((message, oams_name))

    fps_state = helper.current_state.fps_state[fps_name]
    ...
    start_extruder = 10.0
    start_encoder = 5
    fps_state.prime_clog_tracker(start_extruder, start_encoder, 0.46, now - helper.clog_settings["dwell"] - 1.0)
    fps_state.clog_start_time = now - helper.clog_settings["dwell"] - 1.0
    fps_state.clog_start_extruder = start_extruder
    fps_state.clog_start_encoder = start_encoder
    fps_state.clog_min_pressure = 0.45
    fps_state.clog_max_pressure = 0.49

    fps = DummyFPS(position=start_extruder + helper.clog_settings["extrusion_window"] + 5.0)
    ...
    assert pause_messages and pause_messages[0][1] == oams_name
    assert dummy_oams.led_calls == [(2, 1)]
    assert dummy_oams.follower_calls == [(0, 1), (1, 1)]
    assert fps_state.following is True
    assert fps_state.clog_restore_follower is False
