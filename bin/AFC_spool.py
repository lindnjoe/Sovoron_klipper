...
class AFCSpool:
    ...
    def handle_connect(self):
        ...
        # FORK: tool-redirect commands (multi-color on fewer lanes)
    ...
    def register_lane_macros(self, lane_obj: AFCLane):
        ...
        self.gcode.register_mux_command('SET_REDIRECT',         "LANE", lane_obj.name, self.cmd_SET_REDIRECT,           desc=self.cmd_SET_REDIRECT_help) # FORK: per-lane tool redirect
    ...
    def cmd_RESET_AFC_MAPPING(self, gcmd):
        ...
        # FORK: clear any active tool redirects on mapping reset
        ...
        self.afc.save_vars()
        msg = "Tool mappings reset" + ("" if runout_opt == "no" else " and runout lanes reset")
        if self.afc.allow_tool_redirect:  # FORK
            msg += ", redirects cleared"
        self.logger.info(msg)

    # FORK: tool-redirect commands — let multiple T# commands resolve to one
    # physical lane so a multi-color print can run on fewer lanes. Redirects are
    # session-only and cleared on restart, RESET_AFC_MAPPING, or CLEAR_TOOL_REDIRECTS.
