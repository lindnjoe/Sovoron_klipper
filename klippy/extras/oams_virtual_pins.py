...
from .ams_pin import VirtualInputPin, _norm
        ...
        for sec in cp.sections():
            ...
            for i in range(4):
                for suffix in ('_prep', '_load'):
                    raw_name = f'{name}_lane{i}{suffix}'
                    pin_name = _norm(raw_name)
                    fc = _FakeConfig(self.printer, raw_name)
                    ...
                    for key in {f'ams_pin {pin_name}', f'ams_pin {raw_name}',
                                f'ams_pin:{raw_name}'}:
                        try:
                            self.printer.objects[key] = vpin
                        except Exception:
                            pass
