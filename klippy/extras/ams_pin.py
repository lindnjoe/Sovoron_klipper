...
class AmsPinChip:
    ...
    def cmd_SET_AMS_PIN(self, gcmd):
        pin_name = _norm(gcmd.get('PIN'))
        if pin_name is None:
            raise gcmd.error('PIN is required')
        val = gcmd.get_int('VALUE', 1)
        vpin = self.pins.get(pin_name)
        if vpin is None:
            raise gcmd.error(f'Unknown ams_pin {pin_name}')
        vpin.set_value(val)
    ...
    def cmd_QUERY_AMS_PIN(self, gcmd):
        ...
        if vpin is None:
            raise gcmd.error(f'Unknown ams_pin {pin_name}')
        gcmd.respond_info(f'ams_pin {pin_name}: {int(vpin.state)}')
