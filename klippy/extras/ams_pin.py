...
class AmsPinChip:
    ...
    def register_pin(self, vpin):
        ...
        # Expose the pin by its normalized name, raw config suffix,
        # and with an explicit "ams_pin:" prefix so lookups work no
        # matter how other modules reference the pin.
        # Store under normalized forms so lookups work regardless of
        # capitalization or inclusion of the chip prefix.
        key = _norm(vpin.name)
        self.pins[key] = vpin
        self.pins[f'{CHIP_NAME}:{key}'] = vpin
        raw = getattr(vpin, 'raw_name', vpin.name)
        self.pins[_norm(raw)] = vpin
        self.pins[f'{CHIP_NAME}:{_norm(raw)}'] = vpin
