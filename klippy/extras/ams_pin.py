...
class AmsPinChip:
    ...
    def register_pin(self, vpin):
        ...
        # Expose the pin by both the normalized name and the raw section
        # suffix so lookups succeed even if other code does not normalize
        key = _norm(vpin.name)
        self.pins[key] = vpin
...
class VirtualInputPin:
    ...
    def __init__(self, config):
        ...
        # Expose under a normalized key so lookups succeed regardless
        # of the case used in the configuration file.
        try:
            self.printer.objects[f'{CHIP_NAME} {self.name}'] = self
        except Exception:
            pass
