...
class OAMSManager:
    ...
    def notify_lane_tool_loaded(
        self,
        lane_name: str,
        context: Optional[str] = None,
    ) -> None:
        """Refresh follower state when an AFC lane reports a tool load."""
        ...
        if afc is not None:
            try:
                ...
                logging.exception(
                    "OAMS: Failed to refresh AFC lane cache while handling %s",
                    lane_name,
                )

        canonical_lane = lane_name
        group_name = self._canonical_group_by_lane.get(canonical_lane)
        if group_name is None:
            lowered = lane_name.lower()
            for known_lane, known_group in self._canonical_group_by_lane.items():
                if known_lane.lower() == lowered:
                    canonical_lane = known_lane
                    group_name = known_group
                    break

        if group_name is None:
            logging.debug(
                "OAMS: Unable to map lane %s to a filament group while restoring follower",
                lane_name,
            )
            return
        ...
        fps_name = self.group_fps_name(normalized_group)
        if not fps_name:
            logging.debug(
                "OAMS: No FPS associated with group %s when lane %s reported a tool load",
                normalized_group,
                lane_name,
            )
            return
        ...
        group_obj = self.filament_groups.get(normalized_group)
        loaded_spool = None
        if group_obj is not None:
            try:
                loaded_spool = group_obj.get_loaded_spool()
            except Exception:
                logging.exception(
                    "OAMS: Failed to query loaded spool for group %s while restoring follower",
                    normalized_group,
                )

        if loaded_spool:
            oam, bay_index = loaded_spool
            try:
                fps_state.current_oams = getattr(oam, "name", None)
            except Exception:
                fps_state.current_oams = None
            fps_state.current_spool_idx = bay_index
            fps_state.state_name = FPSLoadState.LOADED
            fps_state.current_group = normalized_group
            fps_state.since = self.reactor.monotonic()
            fps_state.reset_stuck_spool_state()
            fps_state.reset_clog_tracker()
            ...
            fps_state.current_group = normalized_group
            if group_obj is not None and (
                fps_state.current_spool_idx is None
                or fps_state.current_oams is None
            ):
                for oam, bay_index in group_obj.bays:
                    oam_name = getattr(oam, "name", None)
                    if self._lane_by_location.get((oam_name, bay_index)) == canonical_lane:
        ...
        follower_context = context or f"lane {canonical_lane} tool load"
        self._ensure_forward_follower(
            fps_name,
            fps_state,
            follower_context,
