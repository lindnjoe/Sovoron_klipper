...
class OAMSManager:
    ...
    def _get_infinite_runout_target_lane(self, fps_name: str, fps_state: 'FPSState') -> Tuple[Optional[str], Optional[str], bool, Optional[str]]:
        ...
        target_extruder = _normalize_extruder_name(
            getattr(target_lane.extruder_obj, "name", None) if hasattr(target_lane, "extruder_obj") else None
        )

        def _normalize_oams_name(name: Optional[str]) -> Optional[str]:
            if not name:
                return None
            if isinstance(name, str) and ":" in name:
                name = name.split(":", 1)[0]
            if isinstance(name, str) and name.startswith("oams "):
                name = name[5:]
            return name

        same_fps = bool(source_extruder and target_extruder and source_extruder == target_extruder)
        if not same_fps:
            source_unit = getattr(lane, "unit_obj", None)
            target_unit = getattr(target_lane, "unit_obj", None)
            source_oams = _normalize_oams_name(getattr(source_unit, "oams_name", None)) if source_unit else None
            target_oams = _normalize_oams_name(getattr(target_unit, "oams_name", None)) if target_unit else None
            if source_oams is None:
                source_oams = _normalize_oams_name(getattr(lane, "unit", None))
            if target_oams is None:
                target_oams = _normalize_oams_name(getattr(target_lane, "unit", None))
            current_oams = _normalize_oams_name(fps_state.current_oams)
            if source_oams and target_oams and source_oams == target_oams:
                same_fps = True
            elif current_oams and target_oams and current_oams == target_oams:
                same_fps = True
    ...
    def _unload_filament_for_fps(self, fps_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if gcode is None:
                gcode = self.printer.lookup_object("gcode")
                self._gcode_obj = gcode
            max_retries_arg = f" MAX_RETRIES={max_retries}" if max_retries is not None else ""
            gcode.run_script_from_command(
                f"OAMS_UNLOAD_SPOOL OAMS={oams.oams_idx}{max_retries_arg}"
            )
            gcode.run_script_from_command("M400")
            success = True
            message = "Spool unloaded successfully"
        except Exception:
            success = False
        ...
        if not success:
            ...
            try:
                ...
                gcode.run_script_from_command("M400")
                self.reactor.pause(self.reactor.monotonic() + 1.0)
                gcode.run_script_from_command(f"OAMS_UNLOAD_SPOOL OAMS={oams.oams_idx}")
                gcode.run_script_from_command("M400")
                success = True
                message = "Spool unloaded successfully"
            except Exception:
    ...
    def _load_filament_for_lane(self, lane_name: str) -> Tuple[bool, str]:
        ...
        for engagement_attempt in range(max_engagement_retries):
            ...
            for stuck_attempt in range(STUCK_SPOOL_MAX_ATTEMPTS):
                ...
                # The follower motor must be set to forward so filament can flow during load.
                try:
                    gcode = self._gcode_obj
                    if gcode is None:
                        gcode = self.printer.lookup_object("gcode")
                ...
                try:
                    ...
                    gcode.run_script_from_command("M400")
                    success = True
                    message = "Spool loaded successfully"
                except Exception:
                    success = False
                ...
                # STEP 3: Abort the stuck load operation via gcode to clear action_status
                ...
                try:
                    ...
                    if gcode is None:
                        gcode = self.printer.lookup_object("gcode")
                        self._gcode_obj = gcode
                    gcode.run_script_from_command(f"OAMS_ABORT_ACTION OAMS={oam.oams_idx}")
                    gcode.run_script_from_command("M400")
                ...
                try:
                    ...
                    if gcode is None:
                        gcode = self.printer.lookup_object("gcode")
                        self._gcode_obj = gcode
                    gcode.run_script_from_command(f"OAMS_UNLOAD_SPOOL OAMS={oam.oams_idx}")
                    gcode.run_script_from_command("M400")
    ...
    def _trigger_stuck_spool_pause(self, fps_name: str, fps_state: "FPSState", oams: Optional[Any], message: str) -> None:
        ...
        if oams is not None:
            # Use gcode to abort so the command queues safely
            try:
                gcode = self._gcode_obj
                if gcode is None:
                    gcode = self.printer.lookup_object("gcode")
