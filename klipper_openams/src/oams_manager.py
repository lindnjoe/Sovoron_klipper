...
class OAMSManager:
    ...
    def __init__(self, config):
        ...
        self.moonraker_status_changes_only = config.getboolean("moonraker_status_changes_only", True)
    ...
    def _start_moonraker_status_sync(self) -> None:
        ...
        if self._moonraker_status_timer is None:
            ...
            mode = "changes-only" if self.moonraker_status_changes_only else "always"
            self.logger.info(
                f"OpenAMS Moonraker status sync enabled (interval={self.moonraker_status_interval:.1f}s, mode={mode})"
            )
    ...
    def _publish_moonraker_status_snapshot(self, eventtime: float) -> None:
        ...
        result = self._moonraker_client.publish_manager_status(
            snapshot,
            eventtime=eventtime,
            changes_only=self.moonraker_status_changes_only,
        )
        if result == "failed":
