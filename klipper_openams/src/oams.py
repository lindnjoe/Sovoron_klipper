...
"""
    ...
    def load_spool(self, spool_idx):
        ...
        timeout = self.reactor.monotonic() + 90.0

        while self.action_status is not None:
            if self.reactor.monotonic() > timeout:
                self.logger.error(f"OAMS[{self.oams_idx}]: Load operation timed out after 90 seconds")
                self.action_status      = None
    ...
    def unload_spool(self):
        ...
        timeout = self.reactor.monotonic() + 120.0

        while self.action_status is not None:
            if self.reactor.monotonic() > timeout:
                self.logger.error(f"OAMS[{self.oams_idx}]: Unload operation timed out after 120 seconds")
                self.action_status      = None
                self.action_status_code = OAMSOpCode.ERROR_UNSPECIFIED
