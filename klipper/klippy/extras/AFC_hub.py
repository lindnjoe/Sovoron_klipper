    ...
    def state(self):
        ...
        if self.switch_pin.lower() == "virtual":
            ...
            for lane in self.lanes.values():
                ...
                if is_hw_managed:
                    val = bool(getattr(lane, "tool_loaded", False))
                    lane_states.append(val)
                    _dbg.append(f"{lane.name}:tool_loaded={val}")
                else:
                    val = bool(getattr(lane, "_load_state", False))
                    lane_states.append(val)
                    _dbg.append(f"{lane.name}:_load_state={val}(type={unit_type})")
            state = any(lane_states)
            if state:
                logging.getLogger("AFC_hub").info(
                    "Hub %s state=True: lanes=%d detail=[%s]",
                    self.name, len(self.lanes), ", ".join(_dbg)
                )
        return state
