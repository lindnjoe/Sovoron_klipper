...
class OAMSManager:
    ...
    def _schedule_post_load_pressure_check(self, fps_name: str, fps_state: "FPSState") -> None:
        ...
        def _monitor_pressure(self, eventtime):
            ...
            # Ensure follower keeps running so manual extrusion remains available
    ...
    def _reactivate_clog_follower(self, fps_name: str, fps_state: "FPSState", oams: Optional[Any], context: str) -> None:
        """Ensure the follower is running after a clog pause.

        Older logic tried to toggle the follower off and then back on using
        multiple state flags. That churned MCU commands and left the follower
        disabled when retries failed. The simplified version below only tries
        to enable the follower when it is currently off and always clears the
        restore flags so they cannot accumulate across pauses.
        """
        ...
        if not fps_state.following or fps_state.clog_restore_follower:
            self._enable_follower(fps_name, fps_state, oams, direction, context)
    ...
    def _check_clog(self, fps_name, fps_state, fps, oams, encoder_value, pressure, now):
        ...
        if not fps_state.clog_active:
            ...
            # Keep follower available for manual extrusion instead of toggling it off
            if is_printing and not fps_state.following:
