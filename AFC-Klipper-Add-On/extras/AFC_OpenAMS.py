...
class afcAMS(afcUnit):
    ...
    def _resolve_runout_target(self, runout_lane_name: Optional[str]):
        ...
        # Accept lane objects or other non-string values by extracting a name
        if isinstance(runout_lane_name, str):
            raw_target = runout_lane_name
        elif hasattr(runout_lane_name, "name"):
            raw_target = getattr(runout_lane_name, "name", None)
        else:
            try:
                raw_target = str(runout_lane_name)
            except Exception:
                raw_target = None
        ...
        resolved_lane_name = self._resolve_lane_alias(raw_target)

        if resolved_lane_name is None and self.registry is not None:
            normalized_group = self._normalize_group_name(raw_target)
            if normalized_group:
        ...
        if resolved_lane_name is None:
            resolved_lane_name = raw_target

        target_lane = self._get_lane_object(resolved_lane_name)
        if target_lane is None and resolved_lane_name != raw_target:
            target_lane = self._get_lane_object(raw_target)
    ...
    def _resolve_lane_alias(self, identifier: Optional[str]) -> Optional[str]:
        ...
        if isinstance(identifier, str):
            ...
            lookup = str(getattr(identifier, "name", "")).strip()
        else:
            try:
        ...
        for lane in _iter_candidate_lanes():
            lane_name = getattr(lane, "name", None)
            if isinstance(lane_name, str) and lane_name.lower() == lowered:
                return lane_name
