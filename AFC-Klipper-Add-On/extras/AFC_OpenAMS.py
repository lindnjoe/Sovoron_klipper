...
class afcAMS(afcUnit):
    ...
    def _unload_after_td1(self, cur_lane, spool_index, fps_id):
        ...
        for attempt in range(2):
            ...
            unload_start = self.afc.reactor.monotonic()
            unload_deadline = unload_start + 3.0
            resend_deadline = unload_start + 0.4
            ...
            while self.afc.reactor.monotonic() < unload_deadline:
                ...
                if hub_cleared:
                    break
                if not resend_attempted and self.afc.reactor.monotonic() >= resend_deadline:
                    try:
                        self.oams.oams_unload_spool_cmd.send([])
                    except Exception:
                        self.logger.error(
                            f"Failed to resend unload command for {cur_lane.name}"
                        )
