...
class afcAMS(afcUnit):
    ...
    def handle_ready(self):
        ...
        if self.oams is None:
            self.logger.warning(
                ...
                f"If you are using Box Turtle or other non-AMS hardware, "
        ...
        if self.hardware_service is not None:
            ...
            self.logger.warning(f"AMSHardwareService not available, using legacy polling for {self.name}")
    ...
    def handle_runout_detected(self, spool_index: Optional[int], monitor=None, *, lane_name: Optional[str] = None) -> None:
        ...
        if not same_extruder_handoff:
            ...
            if runout_lane_name and not target_lane:
                self.logger.warning(
                    f"Runout handoff lane {runout_lane_name} not found for {lane.name}; deferring to AFC's native runout handling"
                )
            elif target_lane:
            ...
            if callable(runout_cb):
                ...
                self.logger.warning(
                    f"Runout detected for {lane.name} but no extruder runout handler is available; AFC must handle downstream"
                )
            return
    ...
    def _handle_spool_loaded_event(self, *, event_type=None, **kwargs):
        ...
        try:
            ...
            self.logger.debug(f"Failed to refresh temperature for lane {lane.name}")
        ...
        try:
            ...
            self.logger.error(f"Failed to update spool info for {lane.name} after load event")
    ...
    def _handle_spool_unloaded_event(self, *, event_type=None, **kwargs):
        ...
        try:
            ...
            self.logger.debug(f"Failed to refresh cached temperature for unloaded lane {lane.name}")
