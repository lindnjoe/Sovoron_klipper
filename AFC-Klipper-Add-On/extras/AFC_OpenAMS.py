...
def _patch_infinite_runout_handler() -> None:
    ...
    def _ams_perform_infinite_runout(self, *args, **kwargs):
        ...
        # Re-implement AFC's infinite runout sequence here so we can add
        # step-specific diagnostics without relying on the original handler.
        step = "led"
        try:
            self.status = AFCLaneState.NONE
            self.afc.function.afc_led(self.afc.led_not_ready, self.led_index)
            self.logger.info("Infinite Spool triggered for %s", lane_name)

            step = "mark_change_lane"
