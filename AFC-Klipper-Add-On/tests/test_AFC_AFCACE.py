"""
Unit tests for extras/AFC_AFCACE.py

Covers:
  - afcAFCACE: class constants, slot mapping, color conversion
  - AFCACEPersistence: deferred/immediate modes, save/flush
...
from extras.AFC_AFCACE import afcAFCACE, AFCACEPersistence, MODE_COMBINED, MODE_DIRECT
from extras.AFC_lane import AFCLaneState


# ── Helpers ───────────────────────────────────────────────────────────────────

def _make_afcace(name="AFCACE_1", mode=MODE_COMBINED):
    """Build an afcAFCACE bypassing the complex __init__."""
    unit = afcAFCACE.__new__(afcAFCACE)
    ...
    unit.name = name
    unit.full_name = ["AFC_AFCACE", name]
    unit.lanes = {}
    unit.hub_obj = None
...
class TestConstants:
    def test_slots_per_unit(self):
        assert afcAFCACE.SLOTS_PER_UNIT == 4
...
class TestAceColorToHex:
    def test_rgb_conversion(self):
        assert afcAFCACE._ace_color_to_hex([255, 128, 0]) == "#ff8000"

    def test_black_for_zeros(self):
        assert afcAFCACE._ace_color_to_hex([0, 0, 0]) == "#000000"

    def test_fallback_for_short_array(self):
        assert afcAFCACE._ace_color_to_hex([255]) == "#000000"

    def test_fallback_for_non_list(self):
        assert afcAFCACE._ace_color_to_hex("red") == "#000000"

    def test_tuple_input(self):
        assert afcAFCACE._ace_color_to_hex((128, 64, 32)) == "#804020"
...
class TestGetStatus:
    ...
    def test_ace_mode_is_combined(self):
        unit = _make_afcace(mode=MODE_COMBINED)
        status = unit.get_status()
...
class TestPersistenceImmediate:
    def test_save_calls_afc_save_vars(self):
        ...
        afc = MagicMock()
        p = AFCACEPersistence(MockReactor(), afc, MockLogger(), mode="immediate")
        p.save()
...
class TestPersistenceDeferred:
    def test_save_does_not_call_afc_save_vars(self):
        from tests.conftest import MockReactor, MockLogger
        afc = MagicMock()
        p = AFCACEPersistence(MockReactor(), afc, MockLogger(), mode="deferred")
        p.save()
