...
# Stuck-spool / tangle detection (see _check_tangle) is our own implementation
# of an idea from the Kobra-S1/ACEPRO project: watch the ACE firmware's
# continuous feed-assist run time instead of an encoder to spot a jam.
...
class afcACE(afcUnit):
    ...
    def __init__(self, config):
        ...
        # Stuck-spool / tangle detection. A normal buffer refill makes the ACE
        # feed-assist pump run for a fraction of a second; a jammed or tangled
        # spool makes it run continuously. The firmware reports that continuous
        # run time as cont_assist_time, so if it climbs past tangle_pump_time
        # while we're printing we treat the spool as stuck and pause. Encoder-
        # independent and material-agnostic. Off by default (opt-in).
        self._tangle_detection = config.getboolean("tangle_detection", False)
        self._tangle_pump_time = config.getfloat(
            "tangle_pump_time", 4.0, minval=2.0)
        ...
        self.gcode.register_command(
            f'ACE_TANGLE_DETECTION_{unit_suffix}', self.cmd_ACE_TANGLE_DETECTION,
        ...
        for cmd, handler, desc in [
            ...
            ('ACE_TANGLE_DETECTION', self.cmd_ACE_TANGLE_DETECTION, "Enable/disable ACE stuck-spool detection"),
        ]:
            try:
    ...
    def _check_tangle(self, result):
        ...
        blip; a jam keeps the pump running. If that time climbs past
        tangle_pump_time while we're printing with assist on, the spool is
        stuck: stop driving the jam and pause. Runs only on idle heartbeats
        (the caller skips it during load/unload, where long feeds are normal).
        """
        if not self._tangle_detection:
            return
    ...
    def _handle_tangle(self, cont_time):
        ...
        if slot is not None:
            try:
                ...
                pass
        msg = (
            "ACE stuck-spool/tangle detected on {unit}{lane}: feed assist ran "
            "continuously for {t:.1f}s (>= {thr:.1f}s threshold). The spool is "
            "likely tangled or jammed at the unit. Clear the snag, then resume. "
            "Run ACE_TANGLE_DETECTION ENABLE=0 to disable this check.".format(
                unit=self.name,
                lane=" lane {}".format(name) if name else "",
                t=cont_time, thr=self._tangle_pump_time))
        try:
    ...
    cmd_ACE_TANGLE_DETECTION_help = "Enable/disable ACE stuck-spool (tangle) detection"
    def cmd_ACE_TANGLE_DETECTION(self, gcmd):
        """Toggle stuck-spool detection or adjust its threshold at runtime.

        Usage
        -----
        `ACE_TANGLE_DETECTION [ENABLE=0|1] [PUMP_TIME=<seconds>]`
        """
        enable = gcmd.get_int('ENABLE', None, minval=0, maxval=1)
        if enable is not None:
            self._tangle_detection = bool(enable)
            if not self._tangle_detection:
                self._tangle_tripped = False
        pump = gcmd.get_float('PUMP_TIME', None, minval=2.0)
        if pump is not None:
            self._tangle_pump_time = pump
        self.logger.info(
            "ACE tangle detection {state}: pump_time={t:.1f}s".format(
                state="ON" if self._tangle_detection else "OFF",
                t=self._tangle_pump_time))
