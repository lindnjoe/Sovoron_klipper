...
# multiple units — OpenAMS applies the same guard. Requires the unit section to
# be loaded before its [AFC_lane ...] sections (the standard AFC config order).
...
# (jimmy's AFC_buffer had this). Idempotent; also applied from OpenAMS.
...
# the lane load (F1S) sensor, NOT the hub sensor. ACE/OpenAMS drive the real hub
# HES via switch_pin_callback (-> hub._state), so a hub that has been driven
# should report _state. Hubs never driven (vivid-style) keep the original
# behavior. (loaded_to_hub is a position, unrelated to this sensor state.)
# Idempotent; also applied from OpenAMS.
...
# running first (as our two-phase unload did) — otherwise the ACE/OAMS retract
...
# then let the original run the transport. Idempotent; also applied from OpenAMS.
