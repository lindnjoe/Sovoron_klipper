...
class afc:
    ...
    def TOOL_LOAD(self, cur_lane: AFCLane, purge_length: int=None, set_start_time=False):
        ...
        if cur_lane.name != self.current:
            ...
            # `or cur_lane.loaded_to_hub` — a lane pre-staged to its OWN hub
            ...
            # reports "Hub not clear". load_sequence already trusts loaded_to_hub
            # (it skips move-to-hub), so the lane's own staged filament is not an
            # obstruction.
